...
import seederData from "./trainingProgramOwnership.seeder.json";
...
const TrainingProgramOwnershipSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("trainingProgramOwnership").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(TrainingProgramOwnershipSeederDialogComponent);
