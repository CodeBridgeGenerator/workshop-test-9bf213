...
import trainerDetailsFakerFactory from "./trainerDetailsFakerFactory";
...
const TrainerDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = trainerDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("trainerDetails").create(elem));
...
export default connect(mapState, mapDispatch)(TrainerDetailsFakerDialogComponent);
