...
import seederData from "./trainerDetails.seeder.json";
...
const TrainerDetailsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("trainerDetails").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(TrainerDetailsSeederDialogComponent);
