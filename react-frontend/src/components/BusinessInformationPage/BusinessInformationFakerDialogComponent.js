...
import businessInformationFakerFactory from "./businessInformationFakerFactory";
...
const BusinessInformationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = businessInformationFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("businessInformation").create(elem));
...
export default connect(mapState, mapDispatch)(BusinessInformationFakerDialogComponent);
