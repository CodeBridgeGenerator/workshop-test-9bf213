...
import seederData from "./businessInformation.seeder.json";
...
const BusinessInformationSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("businessInformation").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(BusinessInformationSeederDialogComponent);
