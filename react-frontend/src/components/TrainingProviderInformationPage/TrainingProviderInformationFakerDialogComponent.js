...
import trainingProviderInformationFakerFactory from "./trainingProviderInformationFakerFactory";
...
const TrainingProviderInformationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = trainingProviderInformationFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("trainingProviderInformation").create(elem));
...
export default connect(mapState, mapDispatch)(TrainingProviderInformationFakerDialogComponent);
