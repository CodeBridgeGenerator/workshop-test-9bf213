...
import seederData from "./trainingProviderInformation.seeder.json";
...
const TrainingProviderInformationSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("trainingProviderInformation").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(TrainingProviderInformationSeederDialogComponent);
