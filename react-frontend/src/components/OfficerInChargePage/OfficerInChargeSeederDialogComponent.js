...
import seederData from "./officerInCharge.seeder.json";
...
const OfficerInChargeSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("officerInCharge").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(OfficerInChargeSeederDialogComponent);
