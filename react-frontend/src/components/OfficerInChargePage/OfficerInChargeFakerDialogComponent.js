...
import officerInChargeFakerFactory from "./officerInChargeFakerFactory";
...
const OfficerInChargeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = officerInChargeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("officerInCharge").create(elem));
...
export default connect(mapState, mapDispatch)(OfficerInChargeFakerDialogComponent);
