...
const OfficerInChargeCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,icNumber: _entity?.icNumber,designation: _entity?.designation,email: _entity?.email,telephoneNumber: _entity?.telephoneNumber,organizationName: _entity?.organizationName,
